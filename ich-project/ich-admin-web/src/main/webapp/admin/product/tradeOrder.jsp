...
<body>
			...
			<div style="overflow:hidden;">
				...
				<div style="float:right;padding:5px 0;">
					<form action="" id="f_search" style="display:inline-block;">
						...
						<input name="entrustCode" type="text" class="form-control input-sm dis-i-b w-auto" placeholder="委托编号" />
					</form>
		...
		<form action="" class="e-form" method="post">
			<ul class="e-table" border="0">
				<li>
					...
					<p>开始时间:&nbsp;</p>
					<input type="text" class="form-control input-sm" name="startTime" param-index="1" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'});" />
				</li>
				<li>
					<p>结束时间:&nbsp;</p>
					<input type="text" class="form-control input-sm" name="endTime" param-index="2" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'});" />
				</li>
			</ul>
		</form>
	...
	<script type="text/javascript">                                                   
		homePath = "${ctx}";                                                          
		findUrl = homePath + "/admin/product/tradeOrder/find";                        
		saveUrl = homePath + "/admin/product/tradeOrder/save";                        
		deleteUrl = homePath + "/admin/product/tradeOrder/delete";                    
		entityName = "交易订单";                                                      
		columns = [                                                                   
			{ data: "id" },                                                           
			{ data: "customerNo" },                                                   
			{ data: "productName" },                                                  
			{ data: "tradeMarkString" },
			{ data: "entrustPrice" },
			{ data: "entrustNumber" },
			{ data: "entrustCode" },
			{ data: "turnoverQuantity" },
			{ data: "priceMethodString" },
			{ data: "entrustMethodString" }
		];
		...
		$(function(){
			$("#festival_index").css("background-color","#ffffff");
			$("#content10").show();
		});
	</script>
</body>
